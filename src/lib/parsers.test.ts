...
vi.mock('epubjs', () => ({ default: () => ({ ready:Promise.resolve(), loaded:{ metadata:Promise.resolve({ title:'E Book', creator:'Author' }), navigation:Promise.resolve({ toc:[{ href:'one.xhtml', label:'One' }] }) }, spine:{ spineItems:[{ href:'one.xhtml', load:async () => ({ body:{ innerText:'EPUB body text' } }), unload:vi.fn() }] }, load:vi.fn(), destroy:vi.fn() }) }))
